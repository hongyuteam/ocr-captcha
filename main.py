...
@app.route('/captcha/base64', methods=['POST'])
def get_captcha_base64():
    # Base64编码，不要包含[data:image/png;base64,]的开头
    img_base64 = request.form.get('image')
    # print("img_base64: {}".format(img_base64))
    img_bytes = base64.b64decode(img_base64)
    # 识别验证码
    ocr = ddddocr.DdddOcr(show_ad=False)
    code = ocr.classification(img_bytes)
    # print("识别到的内容 {}".format(code))
...
def get_captcha():
    # http://127.0.0.1:9999/captcha -> form-data: file
    # filepath = request.args.get('filepath')
    filepath = request.form.get('filepath')
    # print(f'request.form.filepath: {filepath}')
    ...
    if file is not None:
        ...
        root_path = os.path.join(os.getcwd(), 'runtime')
        if not os.path.exists(root_path):
            os.makedirs(root_path)
        filename = str(uuid.uuid1()) + '_captcha.' + file_suffix
